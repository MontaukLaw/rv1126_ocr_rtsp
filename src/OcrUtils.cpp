...
std::vector<ImgBox> getImage(std::vector<TextBox> box, cv::Mat &src)
{
    ...
    for (int i = 0; i < box.size(); i++)
    {
        ...
        crop_img.emplace_back(ImgBox{img_warp, box[i].boxPoint,
                                     box[i].boxPoint[0].x + img_w / 2,
                                     box[i].boxPoint[0].y + img_h / 2});
    }
    return crop_img;
}
...
{
    // 需要画框可以打开下面的代码
    // auto color = cv::Scalar(0, 255, 0);// R(255) G(0) B(0)
    ...
    // 获取box的中心点
    // int x = (box[0].x + box[2].x) / 2;
    // int y = (box[0].y + box[2].y) / 2;

    // 画多边形的中心点
    cv::circle(boxImg, cv::Point(x, y), 8, cv::Scalar(255, 0, 0), -1);
}
